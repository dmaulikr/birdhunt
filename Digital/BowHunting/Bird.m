...
-(id) initWithSpeed:(float) sp
{
	...
	if( (self=[super init]) ) {
        ...
        //self.ani=[CCSprite spriteWithSpriteFrameName:@"birdFly0001.png"];
...
-(void) reset{
    ...
    if(arc4random()%10>=5){
        ...
        //go left
        speed=-speed;
        self.position=ccp(ws.width+self.ani.contentSize.width/2+self.overDis,arc4random()%200+100);
        self.scaleX=1;
        ...
        self.position=ccp(-self.ani.contentSize.width/2-self.overDis,arc4random()%200+100);
...
-(void)loop{
    ...
    if(!self.isDead){
        self.position=ccp(self.position.x+self.speed+((((GameMain*)self.gm).currentLevel*0.1)*(self.speed>0?1:-1)),self.position.y);
        if(self.speed>0&&self.position.x>=ws.width+self.ani.contentSize.width/2+self.overDis){
            self.speed*=-1.0f;
            self.scaleX*=-1.0f;
            self.position=ccp(self.position.x,arc4random()%200+100);
        }
